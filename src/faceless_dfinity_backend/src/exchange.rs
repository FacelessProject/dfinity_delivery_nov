...
use crate::types::*;
use crate::{utils};
use aibe::bf_ibe::{BFIbe, CipherText};
use rand_chacha::ChaCha20Rng;

#[derive(Default)]
pub struct Balances(pub HashMap<String, CipherText>); // owner -> amount
...
impl Balances {
    pub fn get_balance(&self, owner: String) -> Option<CipherText> {
        self.0.get(&owner).cloned()
    }

    pub fn get_balance_mut(&mut self, owner: String) -> Option<&mut CipherText> {
        self.0.get_mut(&owner)
    }

    pub fn add_balance(&mut self, owner: String, delta: CipherText) {

        if let Some(x) = self.0.get_mut(&owner) {
            *x = BFIbe::<ChaCha20Rng>::add_ciphers(&x, &delta);
        } else {
            self.0.insert(owner, delta);
        }
    }
