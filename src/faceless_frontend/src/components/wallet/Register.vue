...
const getHRIPlatforms = async () => {
  ...
  for (let i in platforms.value) {
    platforms.value[i].validated = window.localStorage.getItem(platforms.value[i].name+"-reg") !== null;
    console.log(platforms.value[i].name, ": ", platforms.value[i].validated);
    validated_count += platforms.value[i].validated;
...
const userCreateHRIPlatform = (name: string) => {
  if (name === "Twitter") {
    window.location.href = `https://oauth.faceless.live/auth/twitter?auth_origin_url=${encodeURI("http://127.0.0.1:80")}/omniauth/twitter/registercallback`;
    return;
  }

  if (name === "Google") {
    window.location.href = `https://oauth.faceless.live/auth/google_oauth2?auth_origin_url=${encodeURI("http://127.0.0.1:80")}/omniauth/google_oauth2/registercallback`;
    return;
  }
  ...
  if (name === "Telegram") {
    window.Telegram.Login.auth({ bot_id: '6290560220', request_access: 'write', embed: 1 }, (data) => {
      ...
      if (!data) {
      //失败时你需要做的逻辑
        window.location.href = `http://127.0.0.1:80/omniauth/cancel/registercallback`;
        return
      }
      
      window.location.href = `http://127.0.0.1:80/omniauth/telegram/registercallback?nickname=${data.username}`;
      return
    });
  }
...
<template>
  <div id="AddHRIPlatform">
    <span class="title" v-if="registered">
      You have registered successfully!
    </span>
    <span class="title" v-else>
      Select 3 HRI platforms for registration
    </span>
