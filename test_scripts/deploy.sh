...
dfx deploy ledger --argument '(record  {
    ...
    initial_values = vec { record { "'${LEDGER_ACC}'"; record { e8s=100_000_000_000_000 } }; record { "'${ANONYMOUS_ACC}'"; record { e8s=100_000_000_000_000 } };};
    send_whitelist = vec {}
    })'
...
dfx canister call ledger transfer "(record { amount = record { e8s = 10_000_000_000_000 }; to = $AGENT_DEPOSIT_ADDR; fee = record { e8s = 10000}; memo = 1;})"
...
dfx canister call ledger transfer "(record { amount = record { e8s = 10_000_000_000_000 }; to = $ANONYMOUS_DEPOSIT_ADDR; fee = record { e8s = 10000}; memo = 1;})"
